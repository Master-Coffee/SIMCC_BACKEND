...
# ● incite_generic_data
def incite_generic_data(year, incite_program_id, _dep_id):
    ...
    if INCITE_program_id:
        filter_incite_program = f"""
            AND researcher_id IN (SELECT researcher_id FROM incite_graduate_program_researcher WHERE incite_graduate_program_id = '{INCITE_program_id}')
            """
    else:
