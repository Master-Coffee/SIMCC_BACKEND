...
# ★ /incite_lists_word_researcher
@researcherTermRest.route("/incite_lists_word_researcher", methods=["GET"])
def incite_lists_word_researcher():
    researcher_id = request.args.get("researcher_id")
    dep_id = request.args.get("dep_id")
    incite_program_id = request.args.get("incite_program_id")

    lists_word = list()
    df_bd = termFlowSQL.incite_lists_word_researcher_db(
        researcher_id, incite_program_id, dep_id
    )
